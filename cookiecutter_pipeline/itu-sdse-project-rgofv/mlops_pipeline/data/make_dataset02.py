...
# Internal imports
from mlops_pipeline.config import DATA_DIR, RAW_DATA_DIR, INTERIM_DATA_DIR, PRINTING_STATE, MODELS_DIR, PROCESSED_DATA_DIR
...
def make_dataset(raw_csv_path: Path, out_training_csv_path: Path, min_date: str, max_date: str, printing: bool) -> None:
    '''
    ...
    (INTERIM_DATA_DIR / "metadata").mkdir(parents=True, exist_ok=True)
    outlier_summary.to_csv(INTERIM_DATA_DIR / "metadata" / "outlier_summary.csv")
    if printing:
        outlier_summary

    # Missing values:
    cat_missing_impute = cat_vars.mode(numeric_only=False, dropna=True)
    cat_missing_impute.to_csv(INTERIM_DATA_DIR / "metadata" / "cat_missing_impute.csv")
    if printing:
        cat_missing_impute
    ...
    scaler_path = INTERIM_DATA_DIR / "temp_scaler.pkl"
